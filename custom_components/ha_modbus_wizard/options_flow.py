    ...
    async def async_step_add_register(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                vol.Optional("offset", default=0.0, advanced=True): vol.Coerce(float),
                vol.Optional("options"): str,  # JSON mapping for SelectEntity

                vol.Optional("byte_order", default="big", advanced=True): selector.SelectSelector(
                    selector.SelectSelectorConfig(options=["big", "little"])
                ),
                vol.Optional("word_order", default="big", advanced=True): selector.SelectSelector(
                    selector.SelectSelectorConfig(options=["big", "little"])
                ),
                vol.Optional("allow_bits", default=False, advanced=True): bool,
                # NumberEntity bounds
                vol.Optional("min", advanced=True): vol.Coerce(float),
                vol.Optional("max", advanced=True): vol.Coerce(float),
                vol.Optional("step", default=1, advanced=True): vol.Coerce(float),
            }),
            errors=errors,
        )
